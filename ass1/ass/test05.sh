...
pushy-rm test.txt > "$actual_output" 2>&1

if ! diff "$expected_output" "$actual_output"; then
    echo "Failed test: pushy-rm"
    exit 1
fi

cat > "$expected_output" <<EOF
pushy-rm: error: 'test.txt' is not in the pushy repository
EOF
...
echo "Passed test: pushy-rm (not in the pushy repository)"
exit 0
